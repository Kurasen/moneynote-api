    ...
    private void refundBalance(BalanceFlow flow) {
        ...
        if (flow.getConfirm() && flow.getAccount() != null) {
            ...
            if (flow.getType() == FlowType.EXPENSE) {
                ...
                // 如果是转账类型
                Account toAccount = flow.getTo();// 获取转入账户
                BigDecimal amount = flow.getAmount();// 获取转出金额
                BigDecimal convertedAmount = flow.getConvertedAmount();// 获取转入金额

                // 转出账户增加金额，转入账户减少转换后的金额
